// g-sheet ชื่อ check Time Clock_botNative เชื่อมโยงกับ script ชื่อ code.gs และ Time
function doGet(e) {
...
function getEmployees() { 
 ...
 // var employeeSheet = ss.getSheetByName("Student_IT65"); 
  var employeeSheet = ss.getSheetByName("NAME_Farm"); 
  var getLastRow = employeeSheet.getLastRow();  
 // return employeeSheet.getRange(2, 1, getLastRow - 1, 1).getValues();  
  return employeeSheet.getRange(2, 2, getLastRow - 1, 1).getValues();  
}
...
function clockIn(employee) {
  ...
  for (var j = 2; j <= lastRow; j++)
  {
    ...
    {
      error = 'ต้องกด เวลาออก ก่อน กดเวลาเข้าใหม่ ';
...
function clockOut(employee) {
  ...
  {
    return_array.push(['จำเป็นต้องกด เวลาเข้า ก่อน', '', employee]);
    return return_array; 
  }
